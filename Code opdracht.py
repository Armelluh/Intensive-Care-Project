...
print(f'This patient stayed at the ICU for {lengthStay_hours} hours')
...
patWeight = TSignals.loc[(TSignals['ParameterID'] == 80) & (TSignals['PatientID'] == allPatients[0]), 'Value']  # Fill in condition
print(patWeight)
...
weightTimes = TSignals.loc[(TSignals['ParameterID'] == 80) & (TSignals['PatientID']== allPatients[0]), 'Time']
...
values = TSignals.loc[(TSignals['ParameterID'] == 27)&(TSignals['PatientID']== 107032),'Value']
time_raw = TSignals.loc[(TSignals['ParameterID'] == 27)&(TSignals['PatientID']== 107032),'Time'] # Use a temporary variable for raw time

# Convert admission date string to datetime object once
admDate_dt = pd.to_datetime(admDate)
...
# For example bij adding: .astype("timedelta64[s]").astype(float) / 3600.
# This depends on how you loaded the dates in earlier parts.
time = (pd.to_datetime(time_raw) - admDate_dt).dt.total_seconds() / 3600

# Get the unit of cardiac output from TUnits, to use as y-label
signal_unitID_scalar = TParameters.loc[TParameters['ParameterID'] == 27,'UnitID'].values[0]
signal_unit = TUnits.loc[TUnits['UnitID'] == signal_unitID_scalar,'UnitName'].values[0]
...
signal_addition = TUnits.loc[TUnits['UnitID'] == signal_unitID_scalar,'Addition'].values[0]
signal_multiplier = TUnits.loc[TUnits['UnitID'] == signal_unitID_scalar,'Multiplier'].values[0]
...
plt.figure()
plt.plot(time, values, marker='o', linestyle='-', label=signal_name) # Plot with calculated time and values
plt.xlabel('Time from Admission (hours)') # Corrected x-axis label
plt.ylabel(signal_unit)
plt.title(f'{signal_name} for Patient {allPatients[0]}') # Enhanced title
plt.legend()
...
plt.xlim([0, lengthStay_hours]) # Set x-axis limit based on total stay
plt.grid(True)
plt.show()
...
for iprm in range(len(parameterIDs)):
    if not TSignals.loc[(TSignals['ParameterID'] == parameterIDs[iprm])&(TSignals['PatientID']== 107032),'Value'].empty:
      values = TSignals.loc[(TSignals['ParameterID'] == parameterIDs[iprm])&(TSignals['PatientID']== 107032),'Value']

      time_raw = TSignals.loc[(TSignals['ParameterID'] == parameterIDs[iprm])&(TSignals['PatientID']== 107032),'Time']
      time = (pd.to_datetime(time_raw) - admDate_dt).dt.total_seconds() / 3600

      signal_unitID_scalar = TParameters.loc[TParameters['ParameterID'] == parameterIDs[iprm],'UnitID'].values[0]
      signal_unit = TUnits.loc[TUnits['UnitID'] == signal_unitID_scalar,'UnitName'].values[0]
      ...
      axes.append(ax)
      ax.plot(time,values, linestyle='-', label=signal_name)
      ax.set_title(signal_name)
      ax.set_xlabel('Time from Admission (hours)') # Corrected x-axis label
...
# # interest to monitor the hemodynamic status of a patient. Examples are the
...
# idcs = []
# startTime = TPending.loc[idcs, 'StartTime'].values - admDate
# endTime = []
# values = []
#
...
#     signal_timeUnitID = []
#     signal_unit_time = []
#     signal_dripUnit_multiplier = []
#     signal_dripUnit_addition = []
#
...
# if len(signal_plannedOrder) > 0:
#     signal_continuous = TPlannedOrders.loc[TPlannedOrders['PlannedOrderID'] == signal_plannedOrder[0], 'Continuous'].values[0]
# else:
